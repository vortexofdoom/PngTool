...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        while let Ok(_) = reader.read_exact(&mut buf) {
            let chunk_size = u32::from_be_bytes(buf) + 8;
            let mut rest = vec![0u8; chunk_size as usize];
            reader.read_exact(&mut rest)?;
            let mut data = buf.to_vec();
            data.append(&mut rest);
            chunks.push(Chunk::try_from(data.as_slice())?);
        }
        Ok(Png { chunks })
    }
}
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let indices: Vec<_> = self
            ...
            .map(|(i, _)| i)
            .collect();
        if let Some(&chunk) = indices.get(0) {
            Ok(self.chunks.remove(chunk))
        } else {
            Err(anyhow!("Could not find chunk type to remove"))
        }
    }
